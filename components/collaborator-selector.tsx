...
const sampleCollaborators: Collaborator[] = [
  { id: "1", name: "Alex Johnson", email: "alex@example.com", color: "bg-green-500" },
  { id: "2", name: "Sam Wilson", email: "sam@example.com", color: "bg-blue-500" },
  { id: "3", name: "Taylor Smith", email: "taylor@example.com", color: "bg-purple-500" },
  { id: "4", name: "Jordan Lee", email: "jordan@example.com", color: "bg-yellow-500" },
  { id: "5", name: "Casey Brown", email: "casey@example.com", color: "bg-pink-500" },
  { id: "6", name: "Riley Garcia", email: "riley@example.com", color: "bg-indigo-500" },
  { id: "7", name: "Morgan Davis", email: "morgan@example.com", color: "bg-red-500" },
  { id: "8", name: "Jamie Miller", email: "jamie@example.com", color: "bg-orange-500" },
  { id: "9", name: "Quinn Wilson", email: "quinn@example.com", color: "bg-teal-500" },
]

export function CollaboratorSelector({ selectedCollaborators, onCollaboratorsChange }: CollaboratorSelectorProps) {
  const [open, setOpen] = React.useState(false)

  const handleSelect = (collaborator: Collaborator) => {
    if (selectedCollaborators.some((c) => c.id === collaborator.id)) {
      onCollaboratorsChange(selectedCollaborators.filter((c) => c.id !== collaborator.id))
    } else {
                ...
                {sampleCollaborators.map((collaborator) => (
                  ...
                  >
                    <Check
                      className={cn(
                        "mr-2 h-4 w-4",
                        selectedCollaborators.some((c) => c.id === collaborator.id) ? "opacity-100" : "opacity-0",
                      )}
                    />
          ...
          {selectedCollaborators.map((collaborator) => (
            ...
            >
              <Avatar className={`mr-1 h-4 w-4 ${collaborator.color}`}>
                <AvatarFallback className="text-[10px]">{collaborator.name.charAt(0)}</AvatarFallback>
              </Avatar>
              {collaborator.name}
              <Button
                variant="ghost"
